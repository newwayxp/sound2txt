...
echo  (faster-whisper のダウンロードに数分かかる場合があります)
echo.
...
if %errorlevel% equ 0 (
    ...
    if %errorlevel% neq 0 (
        ...
        echo  ※ ffmpeg がないと音声ファイルの変換に失敗します
    ) else (
...
:: ─────────────────────────────────────────────
:: Step 4: 起動ファイルとショートカットを作成
:: ─────────────────────────────────────────────
echo.
echo [4/4] 起動ファイルを作成しています...

:: run.bat 作成（ダブルクリックで起動）
(
    echo @echo off
    echo cd /d "%%~dp0"
    echo python -X utf8 ui.py
...
if %ERROR% equ 0 (
    ...
    echo  ^|                                         ^|
    echo  ^|  起動方法:                              ^|
    echo  ^|    デスクトップの Sound2Text アイコン  ^|
    echo  ^|    または run.bat をダブルクリック     ^|
    echo  ^|                                         ^|
    echo  ^|  初回起動時に Whisper モデルを         ^|
    echo  ^|  ダウンロードします（約240MB）         ^|
    echo  +=========================================+
) else (
    echo  +=========================================+
    echo  ^|  エラーが発生しました。               ^|
