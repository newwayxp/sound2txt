...
if sys.platform != "win32":
    _LOOPBACK_NAMES = ["blackhole", "soundflower", "loopback audio"]

    def select_active_device(pa: pyaudio.PyAudio) -> tuple[int, dict]:
        """
        Find a system-audio loopback input device on macOS/Linux.

        Priority:
          1. BlackHole / Soundflower / Loopback Audio (by name)
          2. Default input device (fallback with warning)
        """
        for i in range(pa.get_device_count()):
            info = pa.get_device_info_by_index(i)
            if info["maxInputChannels"] > 0:
                if any(lb in info["name"].lower() for lb in _LOOPBACK_NAMES):
                    print(f"[device_utils] Loopback device: {info['name']}")
                    return i, info

        # Fallback: default input with warning
        try:
